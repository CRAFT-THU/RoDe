...
int main(int argc,char **argv) {
    ...
    for(int i = 0; i < ITER; ++i)
        RoDeSpmm_n32_nobs(c_sm.n_segs,c_sm.n_segs_residue,c_sm.Columns(),n,
    ...
    SPC::CudaSparseMatrix<float> c_sm1(sm1);
    m = c_sm1.Rows(), k = c_sm1.Columns(), n = BN;


    cudaDeviceSynchronize();
    cudaEventRecord(event1,0);
    for(int i = 0; i < ITER; ++i)
        RoDeSpmm_n32_nopl(c_sm1.n_segs,c_sm1.n_segs_residue,c_sm1.Columns(),n,
                        c_sm1.Values(),c_sm1.ColumnIndices(),c_sm1.RowOffsets(),
                       c_sm1.seg_row_indices,c_sm1.seg_row_indices_residue,c_sm1.seg_st_offsets,
    ...
    cudaEventRecord(event1,0);
    for(int i = 0; i < ITER; ++i)
        RoDeSpmm_n32(c_sm1.n_segs,c_sm1.n_segs_residue,c_sm1.Columns(),n,
                        c_sm1.Values(),c_sm1.ColumnIndices(),c_sm1.RowOffsets(),
                       c_sm1.seg_row_indices,c_sm1.seg_row_indices_residue,c_sm1.seg_st_offsets,
                       d_B.Values(),d_C2,stream1,stream2);
    cudaEventRecord(event2,0);
