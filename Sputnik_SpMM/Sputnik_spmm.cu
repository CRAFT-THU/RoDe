...
struct SpmmKernel {
    ...
    static __device__ __forceinline__ void KernelFn(
        ...
        __align__(16) Ftype output_fragment[kOutputFragmentSize] = {};
        ...
        if (nonzeros >= Config::kBlockItemsK) {
            ...
            sparse_tile_loader.Load_();
            barrier.Sync();

            memory_aligner.MaskPrefix(values_tile, column_indices_tile);
            barrier.Sync();

            dense_tile_loader.Load_(predicates_n);
        ...
        OutputTile output_tile_storer(m_index, n_index, n, threadIdx.x,
                                    output_fragment, out);
        output_tile_storer.Store_(predicates_n);
...
cudaError_t CudaSpmmBiasRelu(
  ...
  if ((n % 4) == 0) {
    if (n == 8) {
      ...
      // No predicates in the n-dimension.
    //   printf("Choose this kernel ?\n");
    //   ScalarValue, SparseValue, DenseValue, kBlockY,kBlockK,kBlockX,kBlockWidth, Residue, Predicate,launch bound,Minimum occupancy
...
cudaError_t SputnikSpmm(int m, int k, int n, int nonzeros,
                     ...
                     double* __restrict__ output_matrix, cudaStream_t stream) {
    if (n == 32) {
      // No predicates in the n-dimension.
    //   printf("Choose this kernel ?\n");
    //   ScalarValue, SparseValue, DenseValue, kBlockY,kBlockK,kBlockX,kBlockWidth, Residue, Predicate,launch bound,Minimum occupancy
      typedef SpmmConfig<double, double4, double4, 4, 32, 32, 8, 4, false> Config;
      return CudaSpmmEx<Config>(m, k, n, nonzeros, row_indices, values,
                                row_offsets, column_indices, dense_matrix, nullptr,
                                output_matrix, stream);
    } else if ((n % 64) == 0) {
      // No predicates in n-dimension. Set kMinOccupancy to 8 to avoid
      // register spilling. Note that we only use this `large-tile` variant
      // if the batch size is divisble by 64.
      typedef SpmmConfig<double, double4, double4, 4, 32, 64, 8, 4, false, true, 8>
          Config;
      return CudaSpmmEx<Config>(m, k, n, nonzeros, row_indices, values,
                                row_offsets, column_indices, dense_matrix, nullptr,
                                output_matrix, stream);
      ...
      // Scalar kernel.
        typedef SpmmConfig<double, double, double, 1, 32, 32, 32> Config;
        return CudaSpmmEx<Config>(m, k, n, nonzeros, row_indices, values,
                                row_offsets, column_indices, dense_matrix, nullptr,
                                output_matrix, stream);
    }
...
// template cudaError_t CudaSpmmEx<SpmmConfig<double, double4, double4, 4, 32, 32, 8, 4, false>>( \
//       int, int, int, int, const int*, const double*, const int*, const int*, \
//       const double*, const float*, double*, cudaStream_t);

// template cudaError_t CudaSpmmEx<SpmmConfig<double, double4, double4, 4, 32, 64, 8, 4, false, true, 8>>( \
//       int, int, int, int, const int*, const double*, const int*, const int*, \
//       const double*, const float*, double*, cudaStream_t);

// template cudaError_t CudaSpmmEx<SpmmConfig<double, double, double, 1, 32, 32, 32>>( \
//       int, int, int, int, const int*, const double*, const int*, const int*, \
//       const double*, const float*, double*, cudaStream_t);
