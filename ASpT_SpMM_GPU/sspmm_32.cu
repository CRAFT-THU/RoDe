...
void ready(int argc, char **argv)
{
	...
	// printf("size : %d x %d , density : %f \n",nr,nc,(double) ne/ ((double) nr * nc));
...
__global__
void printG(int* array,int pos,int n) {
	for(int i=0; i < n; ++i)
		printf("array[%d]:%d\n",pos+i,array[pos+i]);
}
