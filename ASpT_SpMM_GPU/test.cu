...
void ready()
{
    ...
    temp_v[0].row = 0;  temp_v[0].col = 0;  temp_v[0].val = 1; temp_v[0].grp = gran;
    temp_v[1].row = 1;  temp_v[1].col = 1;  temp_v[1].val = 1; temp_v[1].grp = gran;
    temp_v[2].row = 1;  temp_v[2].col = 2;  temp_v[2].val = 2; temp_v[2].grp = gran;
    temp_v[3].row = 2;  temp_v[3].col = 0;  temp_v[3].val = 1; temp_v[3].grp = gran;
    temp_v[4].row = 2;  temp_v[4].col = 3;  temp_v[4].val = 1; temp_v[4].grp = gran;
	temp_v[ne].row = nr;
...
// <<< nr / SBF, SBSIZE>>>
// SBSIZE = 1024 / 8 ; SBF = SBSIZE / 32 ; MFACTOR = 32
__global__
//__launch_bounds__(BSIZE, 2048/BSIZE)
...
void process()
{
  ...
  if(d_flag[0] == 0) {
	...
	//  key : columen_idx, key2 : 30000, val: nz_idx
	#ifdef PP_TIME
		float ptot_ms0;
    ...
    printf( "avg : %f, vari : %f, num_dense : %d\n", avg, vari, num_dense);
	...
	for(int ik=0;ik<ITER;ik++) {
		// kernel fun
        printf("??? <<< %d,%d>>>\n",ss_gridsize.x,ss_blocksize.x);
		spmv_kernel32_ssparse<<<ss_gridsize, ss_blocksize>>>(sc, _csr_v, _csr_e, _csr_ev, _mcsr_cnt, _mcsr_e, _mcsr_list, _vin, _vout);
	}
...
//      fprintf(stdout, "num_diff : %d\n", num_diff);
		...
		printf("num_diff : %d   diff_percent : %f \n",num_diff, (double)num_diff/(nr*sc)*100);
